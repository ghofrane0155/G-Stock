...
public interface IFactureClientRepository extends JpaRepository<FactureClient,Long> {
    ...
    @Query("SELECT f FROM FactureClient f WHERE f.bonCommande.idBonCommande=:bonCommandeId")
    FactureClient findByIdBonCommande(@Param("bonCommandeId")Long bonCommandeId);
